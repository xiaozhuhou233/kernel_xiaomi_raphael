...
differences from the stock/KamiOC configuration:
  ...
  - 90 Hz at 1.5 GHz (not 1.65 GHz) with a distinct FFC:
      ON command:      B9 oscillator, 13 98 tail
      timing-switch:   A9 oscillator, 65 45 tail
...
# plus the 90 Hz D1 and PHY override.
PANELS = {
...
MODES = (
    {"node": 0, "fps": 60, "clock_from_cfg": "clock_60",
     "clock": None, "xfer": None,
     "d1_from_cfg": "d1_60",
     "ffc_on": "A3 B9 A1 4A 00 1A B8",
     "ffc_sw": "A3 B9 A1 4A 00 8A 18",
     "phy_from_cfg": "phy_60",
     "replace_base": True},
    {"node": 1, "fps": 75, "clock": 1_250_000_000, "xfer": 10_829,
     "d1_from_cfg": "d1_75",
     "ffc_on": "A3 A9 A1 4A 00 1A B8",
     "ffc_sw": "A3 A9 A1 4A 00 8A 18",
     "phy_from_cfg": "phy_60",
     "replace_base": False},
    {"node": 2, "fps": 90, "clock": 1_500_040_000, "xfer": 10_108,
     "d1_from_cfg": "d1_90",
     "ffc_on": "A3 B9 A1 4A 00 13 98",
     "ffc_sw": "A3 A9 A1 4A 00 65 45",
     "phy_from_cfg": "phy_90",
     "replace_base": False},
)
...
def apply_mode(block, cfg, mode):
    ...
    # Clock rate + transfer time.
    # The base 60 Hz node has "clockrate = <1100000000>;" with no transfer
    # time line.  For 75/90 Hz, replace the clockrate AND add transfer time.
    # For the base 60 Hz, also add transfer time (HyperOS3 has it).
    clock_old = "qcom,mdss-dsi-panel-clockrate = <%d>;" % cfg["clock_60"]
    if mode["replace_base"]:
        replacements.append((clock_old,
                             "qcom,mdss-dsi-panel-clockrate = <%d>;\n"
                             "\t\t\t\tqcom,mdss-mdp-transfer-time-us = <10108>;" % clock))
    else:
        replacements.append((clock_old,
                             "qcom,mdss-dsi-panel-clockrate = <%d>;\n"
                             "\t\t\t\tqcom,mdss-mdp-transfer-time-us = <%d>;" % (clock, xfer)))

    # Remove any pre-existing transfer-time line so we don't get duplicates
    # when 75/90 Hz nodes are cloned from the already-modified 60 Hz block.
    import re
